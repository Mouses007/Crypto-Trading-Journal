...
export const LEVMAP_PERIODS = { '5m': 5, '15m': 15, '1h': 60, '4h': 240, '1d': 1440 }
...
export class LeverageMapSource {
    ...
    async update(patch) {
        ...
        if (this.symbol === altesSymbol && period === alteRohPeriode && this.rohPunkte.length) {
            this._schneiden()
            this.rebuild()
            this.onStatus?.('ready')
            return
    ...
    rebuild() {
        ...
        const opts = { mid, bucketSize, spanPct: this.spanPct, mmr: this.mmr, tiers: LEVERAGE_TIERS, seed: false }
