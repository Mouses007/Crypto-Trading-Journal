...
export async function useLoadCalendar() {
    ...
    return new Promise(async (resolve, reject) => {
            ...
            */
            ...
            for (let index1 = 0; index1 < calendarizeData.length; index1++) {
                ...
                for (let index = 0; index < element.length; index++) {
                    ...
                    tempData.dateUnix = element2 !== 0 ? dayjs.tz(`${year}-${String(month).padStart(2, '0')}-${String(element2).padStart(2, '0')}`, timeZoneTrade.value).startOf('day').unix() : 0
            ...
            // Compare normalized: both must be startOf('month') in the trade timezone
            const selectedMonthNormalized = dayjs(selectedMonth.value.start * 1000).tz(timeZoneTrade.value).startOf('month').unix()
            if (param1 == selectedMonthNormalized) {
        ...
        resolve()
            ...
            resolve() // resolve anyway so spinner stops
        }
    })
