...
export async function useECharts(param) {
    ...
    for (let index = 1; index <= 2; index++) {
        ...
        if (param == "init") {
            ...
            if (index == 2) {
                // Combine day-level and trade-level satisfaction
                const allSatisfactions = [...satisfactionArray, ...satisfactionTradeArray]
                if (allSatisfactions.length > 0) {
                    let satisfied = allSatisfactions.filter(obj => obj.satisfaction == true || obj.satisfaction == 1).length
        ...
        for (const key of keys) {
            ...
            var pushingChartBarData = () => {
                if (keys.length <= maxChartValues) {
                    ...
                    temp.itemStyle = { color: proceeds >= 0 ? greenColor : redColor }
                    temp.emphasis = { itemStyle: { color: proceeds >= 0 ? greenColorHover : redColorHover } }
                    chartBarData.push(temp)
                } else {
                    chartBarData.push({
                        value: proceeds,
                        itemStyle: { color: proceeds >= 0 ? greenColor : redColor },
                        emphasis: { itemStyle: { color: proceeds >= 0 ? greenColorHover : redColorHover } }
                    })
                }
            }
    ...
    return new Promise((resolve, reject) => {
        ...
        for (const key of keys) {
            ...
            var pushingChartData = () => {
                ...
                if (param1 == "barChart1") {
                    // Profitfaktor >= 1 = grün, < 1 = rot (0 = keine Daten → grün)
                    // APPT >= 0 = grün, < 0 = rot
                    const isGood = selectedRatio.value == "profitFactor"
                        ? (ratio === 0 || ratio >= 1)
                        : ratio >= 0
                    if (keys.length <= maxChartValues) {
                        var temp = {}
                        ...
                        temp.itemStyle = { color: isGood ? greenColor : redColor }
                        temp.emphasis = { itemStyle: { color: isGood ? greenColorHover : redColorHover } }
                        chartData.push(temp)
                    } else {
                        chartData.push({
                            value: ratio,
                            itemStyle: { color: isGood ? greenColor : redColor },
                            emphasis: { itemStyle: { color: isGood ? greenColorHover : redColorHover } }
                        })
                    }
                }

                if (param1 == "barChart2") {
                    // Win Rate: > 50% = grün, <= 50% = rot (0 = keine Daten → grün)
                    const isGood = probWins === 0 || probWins > 0.5
                    if (keys.length <= maxChartValues) {
                        var temp = {}
                        temp.value = probWins
                        temp.label = {}
...
/**
 * Gauge-Chart (Halbkreis) für Prozent-Werte
 * @param {string} elementId - DOM element ID
 * @param {number} value - Wert 0-100
 * @param {string} title - Titel unter dem Gauge
 * @param {Array} colorStops - [{offset: 0, color: '#...'}, ...] optional
 */
export function useGaugeChart(elementId, value, title, colorStops) {
    return new Promise((resolve, reject) => {
        ...
        const option = {
            series: [{
                ...
                axisLine: {
                    lineStyle: {
                        width: 20,
                        color: colorStops || defaultColorStops
                    }
                },
                pointer: {
...
/**
 * Horizontale Balken für Kategorien
 * @param {string} elementId - DOM element ID
 * @param {Array} categories - Kategorie-Namen (y-axis)
 * @param {Array} values - Werte pro Kategorie
 * @param {Array} colors - Farbe pro Balken (optional)
 */
export function useHorizontalBarChart(elementId, categories, values, colors) {
    return new Promise((resolve, reject) => {
        let el = document.getElementById(elementId)
        if (!el) { resolve(); return }
        let myChart = echarts.init(el)
        ...
        const barData = values.map((val, idx) => ({
            value: val,
            itemStyle: {
                color: colors ? colors[idx] : greenColor
            }
        }))
            ...
            series: [{
                ...
                label: {
                    ...
                    formatter: function (params) {
                        return typeof params.value === 'number' && params.value % 1 !== 0
 ...
 * @param {Array} stressValues - Stresslevel-Werte
 */
export function useStressLineChart(elementId, dates, stressValues, options = {}) {
    return new Promise((resolve, reject) => {
        let el = document.getElementById(elementId)
        if (!el) { resolve(); return }
        let myChart = echarts.init(el)
        ...
        const seriesName = options.name || 'Stresslevel'
            ...
            series: [
                {
                    ...
                    areaStyle: {
                        color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
                            { offset: 0, color: lineColor.replace('0.85', '0.3') },
                            { offset: 1, color: lineColor.replace('0.85', '0.02') }
                        ])
                    }
                }
            ]
...
/**
 * Radar-Chart für Vollständigkeitsanalyse
 * @param {string} elementId - DOM element ID
 * @param {Array} indicators - [{name: 'Playbook', max: 100}, ...]
 * @param {Array} values - Werte pro Indikator (0-100)
 */
export function useRadarChart(elementId, indicators, values) {
    return new Promise((resolve, reject) => {
        let el = document.getElementById(elementId)
        if (!el) { resolve(); return }
        let myChart = echarts.init(el)
                ...
                data: [{
                    ...
                    areaStyle: {
                        color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
                            ...
                            { offset: 1, color: 'rgba(72, 199, 142, 0.15)' }
                        ])
                    },
