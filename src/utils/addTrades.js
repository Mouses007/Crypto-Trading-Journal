...
export async function useGetExistingTradesArray(param99, param0) {
    ...
    existingTradesArray.length = 0 // reinitialize

    return new Promise(async (resolve, reject) => {
        try {
            const results = await dbFind("trades", {
                descending: "dateUnix",
...
export async function useImportTrades(param1, param2) {
    return new Promise(async (resolve, reject) => {
        console.log("\nIMPORTING FILE")
        ...
        /****************************
        * CREATE TRADES FROM BITUNIX DATA
        ****************************/
        const create = async () => {
            await createBitunixTrades()
        ...
        const retryFunction = (callback, delay, tries) => {
            if (tries && callback() !== true) {
                ...
                if (!gotExistingTradesArray.value) {
                    spinnerLoadingPage.value = false
                    alert("Could not fetch existing trades in time. Please refresh the page and try again.")
                    return;
                }
                create()
            }
        ...
        if (gotExistingTradesArray.value) {
            ...
            retryFunction(() => gotExistingTradesArray.value, 1000, 10);
        }
...
async function createBitunixTrades() {
    return new Promise(async (resolve, reject) => {
        ...
        tradesData.forEach((row, i) => {
            ...
            let dateUnix
            if (row.DateUTC) {
                dateUnix = dayjs.utc(row.DateUTC).startOf('day').unix()
            } else {
                dateUnix = dayjs().startOf('day').unix()
            }
            ...
            const tradeObj = {
                ...
                grossWinsQuantity: isGrossWin ? 1 : 0,
                grossLossQuantity: isGrossWin ? 0 : 1,
                netWinsQuantity: isNetWin ? 1 : 0,
                netLossQuantity: isNetWin ? 0 : 1,
...
async function getOpenPositionsParse(param99, param0) {
    return new Promise(async (resolve, reject) => {
        ...
        const results = await dbFind("trades", {
            descending: "dateUnix",
            equalTo: { openPositions: true }
        })
        for (let i = 0; i < results.length; i++) {
            const object = results[i];
            if (object.trades) {
                object.trades.forEach(element => {
...
export const useUpdateMfePrices = async(param99, param0, param2) => {
    return new Promise(async (resolve, reject) => {
        ...
        for (let index = 0; index < mfePrices.length; index++) {
            ...
            try {
                const result = await dbCreate("excursions", {
                    mfePrice: element.mfePrice,
    ...
    const checkTradeAccounts = async () => {
        return new Promise(async (resolve, reject) => {
            const updateTradeAccounts = async (param, param2) => {
                await dbUpdateSettings({ accounts: param })
    ...
    const updateOpenPositions = async (param1, param2, param3) => {
        return new Promise(async (resolve, reject) => {
            ...
            const existing = await dbFirst("trades", {
                equalTo: { dateUnix: param2 }
            })

            if (existing) {
                let openPositions = false
                let tempTrades = existing.trades
                ...
                await dbUpdate("trades", existing.objectId, {
