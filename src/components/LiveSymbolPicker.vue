...
import {
    ...
    livePauseInBackground,
    levMapTier, levMapHours, levMapSpanPct, levMapView, levMapThreshold, levMapMmr, levMapProfileW,
...
<template>
    <div class="livePicker">
        ...
        <select v-model.number="liveFrameMs" class="sidebar-select mb-1" :title="t('live.frameTitle')">
            <option v-for="f in FRAME_MS_OPTIONS" :key="f" :value="f">{{ t('live.perColumn', { ms: f }) }}</option>
        </select>
        <label class="liveToggle">
        ...
        <label class="liveToggle" :title="t('live.volumeBarsTitle')">
            <input type="checkbox" v-model="liveShowVolumeBars" class="me-1" />{{ t('live.volumeBars') }}
        </label>
        <label class="liveToggle" :title="t('live.pauseBgTitle')">
            <input type="checkbox" v-model="livePauseInBackground" class="me-1" />{{ t('live.pauseBg') }}
        </label>
        <div v-if="!livePauseInBackground" class="autoRefHint">{{ t('live.pauseBgOffHint') }}</div>
        ...
        <template v-if="variant === 'levmap'">
            ...
            <select v-model.number="levMapHours" class="sidebar-select mb-1" :title="t('levmap.windowTitle')">
                <option v-for="h in LEV_HOURS" :key="h" :value="h">{{ h }} h</option>
            </select>
            ...
            <input v-model.number="levMapMmr" type="number" min="0.001" max="0.05" step="0.001"
                class="sidebar-select" :title="t('levmap.mmrTitle')" />
            <div class="autoRefHint">{{ t('levmap.mmrHint') }}</div>
        </template>
        ...
        <template v-if="variant === 'oi'">
            <label class="fw-lighter mt-2">{{ t('levmap.windowLabel') }}</label>
            <select v-model.number="levMapHours" class="sidebar-select mb-1" :title="t('levmap.windowTitle')">
                <option v-for="h in OI_HOURS" :key="h" :value="h">{{ h }} h</option>
            </select>
        </template>
    </div>
</template>
