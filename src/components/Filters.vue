...
// Im Dashboard: "Trade Abschluss" Tags ausblenden (nur für Playbook relevant)
...
// Dashboard: Flache Liste aller sichtbaren Tags
...
// Dashboard: Einfache Select-Auswahl → selectedTags Array setzen
function inputDashTag(value) {
    if (value === 'all') {
        ...
        // Nur Trades mit diesem einen Tag (ohne "Kein Tag")
        selectedTags.value = [value]
    }
}

// Dashboard: Aktuellen Select-Wert ermitteln
const dashTagSelectValue = computed(() => {
    ...
    // Genau ein Tag gewählt (ohne t000t)
...
<template>
    ...
    <div id="step10" class="col-12 mb-3">
        <div class="dailyCard">
            <div>
                <span v-if="!filtersOpen" v-on:click="filtersClick" class="pointerClass">Filter <i
                ...
                <span v-if="!filtersOpen" class="dashInfoTitle ms-3">
                    ...
                    <span v-show="filters[pageId].includes('grossNet')">{{ selectedGrossNet.charAt(0).toUpperCase() +
                        selectedGrossNet.slice(1) }} Daten |
                    </span>

                    <span v-show="filters[pageId].includes('positions')">
                        <span v-if="positions.length == selectedPositions.length">Alle Positionen |</span>
                    ...
                    <span v-show="filters[pageId].includes('timeFrame')">
                        {{ selectedTimeFrame.charAt(0).toUpperCase() + selectedTimeFrame.slice(1) }} Zeitraum |
                    </span>

                    <span v-show="filters[pageId].includes('ratio')">
                        <span v-if="selectedRatio != 'profitFactor'">{{ selectedRatio.toUpperCase() }}</span><span
                            v-else>Profitfaktor</span> |
                    </span>

                    <span v-show="filters[pageId].includes('tags')">
                        <span v-if="tags.length == selectedTags.length">Alle
                            Tags</span>
                        <span v-else>Ausgewählte Tags</span>
                    </span>

                    <span v-show="filters[pageId].includes('plSatisfaction')">
                        {{ selectedPlSatisfaction == 'satisfaction' ? 'Zufriedenheit' : "PnL" }} Kalender
                    </span>

                </span>

                <span v-else v-on:click="filtersClick" class="pointerClass mb-3">Filter<i
            ...
            <div v-show="filtersOpen" class="row text-center align-items-center">
                ...
                <!-- Date : calendar -->
                <div class="col-12 col-lg-8 mt-1 mt-lg-0 mb-1" v-show="pageId == 'dashboard' && selectedPeriodRange.value != 'all'">
                    <div class="row">
                        <div class="col-5">
                ...
                <!-- Tags: Dashboard = einfaches Select -->
                <div v-if="pageId == 'dashboard'" class="col-6 col-lg-3 mt-1 mt-lg-1" v-show="pageId != 'calendar'">
                    <select v-on:input="inputDashTag($event.target.value)" class="form-select">
                        ...
                        <option v-for="tag in dashboardTags" :key="tag.id" :value="tag.id" :selected="dashTagSelectValue == tag.id">{{ tag.name }}</option>
                    </select>
                </div>

                <!-- Tags: Andere Seiten = Dropdown mit Checkboxen -->
                <div v-else :class="[pageId == 'screenshots' ? 'col-12' : 'col-6', 'dropdown mt-1 mt-lg-1']"
                    ...
                    <ul class="dropdown-menu dropdownCheck">
                        <div>
                            <a class="pointerClass nav-link" v-on:click="selectAllTags"><span
                                    v-if="!allTagsSelected">Alle auswählen</span><span v-else>Alle abwählen</span></a>
                        </div>
                        <hr>
                        <input class="form-check-input mt-1" type="checkbox" value="t000t"
                            v-model="selectedTags">&nbsp;&nbsp;Kein Tag
                        <hr>
                ...
                <div class="col-6 col-lg-3 dropdown" v-show="pageId != 'screenshots' && pageId != 'calendar'">
                    <button class="btn btn-secondary dropdown-toggle" type="button" data-bs-toggle="dropdown"
                        aria-expanded="false">Positionen <span class="dashInfoTitle">({{ selectedPositions.length
                            }})</span></button>
                    <ul class="dropdown-menu dropdownCheck">
