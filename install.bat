...
REM CHECK 3: Visual Studio Build Tools (mit VC++ Toolset)
...
if exist "!VSWHERE!" (
    ...
    if defined VS_INSTALL_PATH (
        ...
        echo   [!!]  VS Build Tools     Installiert, aber VC++ Toolset fehlt
        echo                             Bitte "Desktopentwicklung mit C++" nachinstallieren
        set "MANDATORY_MISSING=1"
    )
) else (
    where cl >nul 2>nul
...
if "!VS_CHECK!"=="1" (
    echo   [OK]  Build Tools        Gefunden ^(mit VC++ Toolset^)
) else (
    echo   [!!]  Build Tools        VC++ Toolset fehlt
    set "STILL_MISSING=1"
)
...
if "!VSBUILD_OK!"=="0" (
    ...
    if exist "!VSWHERE!" (
        ...
        if defined VS_INSTALL_PATH (
            ...
            echo   [!!]  VS Build Tools     VC++ Toolset immer noch nicht gefunden
        )
    ) else (
        where cl >nul 2>nul
        if !errorlevel! equ 0 (
...
powershell -NoProfile -Command "$ws = New-Object -ComObject WScript.Shell; $s = $ws.CreateShortcut('%SHORTCUT%'); $s.TargetPath = 'wscript.exe'; $s.Arguments = '\"\"\"' + '%INSTALL_DIR%\start-hidden.vbs' + '\"\"\"'; $s.WorkingDirectory = '%INSTALL_DIR%'; $s.IconLocation = '%INSTALL_DIR%\src\assets\icon.ico,0'; $s.Description = 'Crypto Trading Journal starten'; $s.Save()" 2>nul
