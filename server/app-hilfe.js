...
Nach dem Import lassen sich Trades in der Tages-Ansicht mit Notizen, Tags, Playbooks, Zufriedenheits-Bewertung und Screenshots anreichern. Bot-Trades (Grid u.ä.) können als eigene Kategorie geführt werden.`,
    ...
    live_analyse: {
        ...
        text: `- **Marktradar** — die Startseite des Modus: ein Kachelraster mit aktuellen Marktdaten. Kacheln: Fear & Greed, BTC-Dominanz, Funding-Raten, Long/Short + Open Interest, RSI-Streudiagramm, Marktübersicht, Rainbow-Chart, 24h-Liquidationen, Trades × Marktregime (die eigene Handelsbilanz je Marktlage), Altcoin-Saison, Pi Cycle Top, Marktmechanik (Regelwerk mit sechs Marktzuständen), Makro (ES/NQ-Futures, DXY, BTC↔Nasdaq-Korrelation, Stablecoin-Fluss), ETF-Fluss (Bestand der Bitcoin-Spot-ETFs in BTC und dessen tägliche Änderung; braucht einen CryptoQuant-Schlüssel, Gratis-Tarif genügt, einzutragen unter Einstellungen → Marktradar) und „Gesamtlage". Kacheln lassen sich ein-/ausblenden, umsortieren und in der Grösse ändern (Zahnrad auf der Seite); ausgeblendete Kacheln werden gar nicht erst geladen. Jede Kachel hat einen Info-Knopf mit Erklärung und Quellenangabe.
...
- **Nachrichten** — KI-Zeitungsbericht (täglich, wöchentlich oder nur auf Knopfdruck, Kapitel Krypto/Finanzen/Tech/Chartanalyse wählbar, Länge kurz/mittel/lang, mit Archiv), Wirtschaftskalender (Zeitraum/Impact/Länder einstellbar) und die Roh-Meldungen der eingerichteten Quellen (RSS, YouTube mit KI-Zusammenfassung, Telegram, X, Truth Social). Das Kapitel „Chartanalyse" trägt zusammen, was Analysten aktuell zur technischen Lage der fünf grössten Coins (nach Marktkapitalisierung, ohne Stablecoins) schreiben — per Recherche inklusive Chart-Bildern aus den Artikeln und, sofern YouTube-Videos Charts besprechen, mit den dort genannten Marken (das Video wird dann als Quelle genannt); es wird nichts selbst gerechnet. Der Zeithorizont ist wählbar: kurzfristig (Analysen des letzten Tages, Intraday-Sicht), mittelfristig (eine Woche) oder übergeordnet (ein Monat) — die Wahl steuert zugleich, wie alt die ausgewerteten Analysen sein dürfen. Jeder Bericht beginnt mit einer Abwägung „Was dafür spricht / Was dagegen spricht / Woran es sich entscheidet"; jede Zeile ist als **Fakt** (steht so in den Quellen) oder **Einschätzung** (gedeutet) markiert, und „Woran es sich entscheidet" nennt beobachtbare Bedingungen — keine Empfehlungen, keine Kursziele. Die Darstellung des fertigen Berichts ist umschaltbar (Dossier ist die Vorgabe · Kombiniert · Zeitung · Kacheln); „Dossier" zeigt zuerst Tabellen — gemessener Marktstand zum Berichtszeitpunkt, die Termine der nächsten 36 Stunden und die wörtlichen Kennzahlen je Kapitel — und darunter die Meldungen mit Bild. Das Umschalten ändert nur die Anzeige, nicht den Bericht, und kostet nichts. Unter KI → Nachrichten gibt es ausserdem ein Feld „Eigene Anweisungen an die KI" (max. 2000 Zeichen): Ton, Schwerpunkte und Ausschlüsse für den Lagebericht, wirksam ab dem nächsten Lauf. Es steuert Stil und Auswahl, hebelt die Grundregeln aber nicht aus — keine Handelsempfehlungen, keine Kursziele, keine Prognosen, nichts Erfundenes. Bewusst nur in den Einstellungen und nicht auf der Nachrichten-Seite. Zusätzlich gibt es **Zwischenmeldungen**: ein- oder zweimal täglich zu frei wählbaren Stunden (KI → Nachrichten → „Zwischenmeldungen"), jederzeit auch von Hand über den Knopf „Aktualisieren" auf der Nachrichten-Seite. Eine Zwischenmeldung schreibt den Tagesbericht NICHT um. Sie durchsucht die eigenen Quellen und — mit Perplexity-Schlüssel — das Netz nach dem, was seit dem Bericht dazugekommen ist: Meldungen, veröffentlichte Zahlen, Beschlüsse, Termine mit Bezug zu Krypto und Handel. Der Tagesbericht wird ihr nur zur Abgrenzung vorgelegt, damit sie nichts wiederholt; korrigiert sie etwas daraus, trägt die Meldung die Marke „korrigiert". Sie fällt eine Stufe kürzer aus als der Tagesbericht (lang → mittel, mittel → kurz) und lässt das Chartanalyse-Kapitel weg, weil Unterstützungen und Widerstände sich in fünf Stunden nicht bewegen. Jede Zwischenmeldung ist ein eigener Eintrag im Archiv; der Tagesbericht bleibt lesbar und ist aus der Zwischenmeldung heraus mit einem Klick erreichbar. Quellen und Filter (u.a. Schlagwort-Filter) werden in den Einstellungen unter KI → Nachrichten gepflegt.
...
**Hype-Radar** — „was ist neu draussen, und was davon hat Substanz?" Er durchsucht sechs Quellen nach jungen Coin-Projekten — CoinGecko, CoinPaprika, DexScreener, GeckoTerminal, pump.fun und Reddit, alle ohne Schlüssel, rechnet eine Hype-Note aus fünf Teilnoten (wie viel bezahlte Sichtbarkeit im Spiel ist, ob der Handel mitzieht, aus wie vielen unabhängigen Belegdomänen der Fund stammt, ob er in ein laufendes Thema passt, wie jung das Handelspaar ist), prüft jeden Kandidaten hart auf Betrugsmuster (GoPlus, bei Solana RugCheck: Honeypot, Mint-Rechte, Halterkonzentration) und lässt erst danach ein Sprachmodell einen Bericht über die wenigen Übriggebliebenen schreiben.
Die Quellen im Einzelnen: **pump.fun** ist die früheste — ein Token auf der Bindungskurve hat noch keinen Handelspool und taucht bei DexScreener und GeckoTerminal deshalb gar nicht auf. **CoinPaprika** ist die zweite Meinung zur Entdeckung, anderes Haus als CoinGecko mit eigenen Aufnahmekriterien. **Reddit** ist die einzige Quelle, in der Menschen reden statt Ketten zu handeln; sie läuft über den RSS-Feed, weil die JSON-Schnittstelle gesperrt ist, und wird auf einen Abruf je Minute gebremst.
Für die Bewertung zählt nicht die Zahl der Quellen, sondern die Zahl unabhängiger BELEGDOMÄNEN: „Handel" (DexScreener, GeckoTerminal, pump.fun), „Entdeckung" (CoinGecko, CoinPaprika, Boost-Listen) und „Gerede" (Reddit). Drei Anbieter derselben Domäne sind EIN Beleg — sie lesen dieselbe Wirklichkeit, nur unterschiedlich vollständig.
Zwei Deckel gehören dazu, beide gewollt: Ein Auftritt in einem Werbe-Unterforum bringt höchstens 60 von 100 Punkten, gekaufte Sichtbarkeit höchstens 30. Wer sich Reichweite kauft, soll nicht in die obere Hälfte kommen. Die überhaupt erreichbare Gesamtnote liegt dadurch bei rund 84 statt 100 — was die Rangfolge nicht verändert, weil es alle Funde gleich trifft, aber erklärt, warum keine Note darüber auftaucht.
CryptoPanic und LunarCrush waren früher ebenfalls vorgesehen und sind am 21.08.2026 entfernt worden: CryptoPanic hat seinen Gratis-Tarif abgeschafft (ab 50 $/Woche), LunarCrush war nie gratis (ab 90 $/Monat).
...
Dazu eine ZWEITE, getrennte Note: die **Ausführungsgüte**. Für jeden übriggebliebenen Coin wird das echte Orderbuch von Bitunix UND Bitget geholt und ausgerechnet, was eine Order über 5 000 USD kostet — Kauf und Verkauf getrennt, denn ein Buch, das den Einstieg billig und den Ausstieg teuer macht, ist eine Falle, die kein Durchschnitt zeigt. Angezeigt wird auch, auf welcher der beiden Börsen es günstiger ist; die Unterschiede sind gross (im Testlauf: TREE 5 Basispunkte auf Bitunix, 54 auf Bitget). Beide Noten stehen NEBENEINANDER und werden nicht verrechnet. Pionex fehlt in dieser MESSUNG, weil für die Börse kein geprüfter öffentlicher Orderbuch-Zugang vorliegt — nicht etwa, weil es keine Perpetuals gäbe; es sind über sechshundert. In der Spalte „handelbar auf" und im Börsenfilter ist Pionex daher vertreten, nur bei Slippage und Tiefe steht nichts.

Dazu eine DRITTE Achse, ebenfalls getrennt: der **Bezug zu Bitcoin**. Sie beantwortet die Frage „kann ich diesen Coin auf meine BTC-Chartanalyse hin mithandeln?" mit zwei Zahlen. Der **Gleichlauf** (Spalte „BTC", in Prozent) sagt, OB der Coin mitläuft; **Beta** sagt, wie kräftig — bewegt sich Bitcoin um 1 %, bewegt sich der Coin im Schnitt um Beta Prozent. Gerechnet wird auf 4h-Kerzen über rund einen Monat, auf Renditen statt Kursen und über Zeitstempel gepaart statt über die Zeilennummer; die Zeiteinheit ist bewusst eine andere als die des Laufs, damit sich der Messzeitraum nicht mitverschiebt, wenn jemand die Einstellung ändert.
Zusätzlich wird geprüft, ob der Zusammenhang überhaupt noch gilt: Der Zeitraum wird halbiert und jede Hälfte einzeln gerechnet. Weichen sie stärker ab, als der Zufall erklärt (Fisher-z über 1,96), erscheint ein Warndreieck. Ein gemessener Fall: über den Monat 54 % Gleichlauf, in der ersten Hälfte aber 82 % und in der zweiten 38 % — die Durchschnittszahl war richtig und als Handelsgrundlage trotzdem wertlos.
Ein Vor-/Nachlaufmass gibt es mit Absicht NICHT: An 18 Coins gemessen lag bei allen 18 die stärkste Übereinstimmung bei Verschiebung null — auf vier Stunden bewegt sich der Markt in derselben Kerze. Nichts davon geht in die Note ein: „bewegt sich viel", „lässt sich günstig handeln" und „folgt Bitcoin" sind drei Fragen. Ein Coin ohne Messung (zu kurz gelistet) bleibt leer und gilt ausdrücklich NICHT als eigenständig.

**Börsenfilter** — über der Tabelle stehen drei Chips: BX (Bitunix), BG (Bitget), PX (Pionex); der volle Name erscheint beim Darüberfahren. Mehrere gewählt heisst „auf mindestens EINER handelbar", nicht „auf allen" — wer Konten bei zweien hat, sucht die Vereinigung. Grundlage sind die Listungen der Börsen und nicht die Ausführungsmessung, weil nur sie den Zustand „unbekannt" kennen: Antwortet eine Börse gerade nicht, bleibt die Zeile stehen und trägt ein Fragezeichen. Am Symbol steht ausserdem leise, auf welchen Börsen es den Coin gibt. Neben den Filtern „Im Spiel", „Trendend" und „Auf beiden Zeiteinheiten" gibt es dafür auch „Läuft mit BTC" und „Eigenständig".
...
- Jede Note lässt sich aufklappen: vier Teilnoten als Balken, die Rohwerte je Zeiteinheit (Vorgabe 1h trägt die Note, 15m bestätigt oder widerspricht), die Ausführungskosten je Börse und der Bitcoin-Bezug (Gleichlauf, Beta, wie viel davon Bitcoin erklärt, beide Zeitraumhälften und wie viele Kerzen verglichen wurden).
- Sortiert wird per Klick auf die Spaltenüberschrift, die Rangspalte „#" eingeschlossen; ein zweiter Klick kehrt die Richtung um. Nicht gemessene Werte stehen dabei IMMER am Ende, in beide Richtungen — sonst sähe ein Coin ohne Messung wie der beste der Liste aus.
...
- **Lernen** — die Sitzung. Oben steht, wie viele Karten heute fällig sind, darunter ein Balken mit der Verteilung über die vier Boxen. „Sitzung starten" zeigt eine Frage; nach „Antwort zeigen" wird selbst bewertet: **Vergessen** (zurück auf Box 1), **Schwer** (bleibt in der Box), **Gut** (eine Box weiter), **Leicht** (zwei Boxen weiter). Je höher die Box, desto seltener kommt die Karte wieder: Box 1 sofort, Box 2 nach einem Tag, Box 3 nach drei, Box 4 nach sieben Tagen. Mit „Vergessen" ODER „Schwer" bewertete Karten kommen in derselben Sitzung noch einmal dran — je Karte genau einmal, damit die Runde nicht endlos wird. Am Telefon oder mit der Maus wird geklickt, am Rechner gehen auch die Tasten 1 bis 4 in der Reihenfolge der Knöpfe.
  Zwei weitere Schaltflächen: Steht bei einer Antwort **„Warum ist das so?"**, klappt darunter eine ausführlichere Erklärung auf — woher eine Schwelle kommt, wie eine Kennzahl gerechnet wird, wo sie in die Irre führt. Bei den Karten zur Marktstruktur (BOS/CHoCH, Order Block, Fair Value Gap, Breaker, Premium/Discount und weitere) steht unter der Antwort zusätzlich eine **Skizze**, die die Definition zeigt; bei zwei Karten kommt darunter ein **echtes Marktbeispiel** aus eingefrorenen Binance-Kerzen, mit Symbol, Zeiteinheit, Datum und der verwendeten Regel. Und **„Karte ausblenden"** in der Kopfzeile blendet die gerade gezeigte Karte aus: sie verschwindet aus der laufenden Runde und zählt nicht mehr als fällig. Gelöscht wird nichts — im Reiter „Karten" lässt sie sich mit einem Klick zurückholen, ihr Lernverlauf ist noch da.
- **Karten** — Verwaltung. Oben die eigenen Karten (anlegen, bearbeiten, löschen), darunter die mitgelieferten. Mitgelieferte Karten lassen sich nicht bearbeiten, aber per Schalter ausblenden — sie zählen dann nicht mehr als fällig, ihr bisheriger Lernverlauf bleibt aber erhalten. Gefiltert wird nach Level: **Level 1** sind Grundbegriffe, ohne die sich nichts anderes lesen lässt, **Level 2** vertiefte Konzepte (On-Chain-Kennzahlen, Derivate-Feinheiten, Risikokennzahlen), **Level 3** Spezialwissen (Orderfluss, Optionen, Börsen-Innenmechanik, Backtest-Fallen, Zyklusmodelle). Der Filter hier wirkt nur auf diese Liste. Für die Sitzung selbst gibt es eine eigene Stufenwahl auf der Lernseite: Über dem Knopf „Sitzung starten" lässt sich auswählen, welche Level in der nächsten Runde drankommen — mit der Zahl der fälligen Karten je Stufe daneben. Ohne Auswahl laufen alle Stufen; die Wahl wird gemerkt.
- **Statistik** — vier Kopfzahlen (Karten insgesamt, gemeisterte Karten in Box 4, Erfolgsquote, Lernserie in Tagen), ein Balkendiagramm der Wiederholungen der letzten 14 Tage und der Erfolg je Kategorie, schwächste zuerst. Kategorien mit sehr wenigen Bewertungen sind ausgegraut und mit „zu wenig Daten" markiert, statt eine Quote zu behaupten, die noch Zufall sein kann. In der Erfolgsquote zählt **„Schwer" nicht als Treffer**: eine Karte, die man nur mit Mühe wusste, kann man noch nicht. Bewertungen von vor dem 05.09.2026 wurden anders gezählt, weshalb die Quote am Anfang etwas zu gut aussehen kann und sich mit jeder neuen Runde zurechtrückt.

Auf der Übersichtsseite gibt es zusätzlich die Kachel **Quiz**: klein zeigt sie nur, wie viele Karten fällig sind; ein Klick vergrössert sie, und dort läuft eine vollwertige Sitzung mit denselben Regeln und denselben Knöpfen wie auf der Lernseite, ohne die Übersicht zu verlassen. Eigene Karten und der Lernfortschritt liegen in der Datensicherung.`,
    },
    einstellungen: {
