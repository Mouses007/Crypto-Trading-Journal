...
export function baueSystem(englisch, empfehlungen = false) {
    ...
    const schema = englisch
        ? [
            ...
            ' "widerspruch": "a tension the text does NOT already state — empty string if there is none",',
            ' "achten": ["at most three concrete things to watch, each with the number that would matter"]}',
            '"widerspruch" is not a summary: never restate the figures of "text" in other words. If the text already',
            'carries the tension, leave it empty — a repetition is worse than an empty field, it doubles every number on the page.',
        ...
        : [
            ...
            ' "widerspruch": "eine Spannung, die im „text" noch NICHT steht — leerer Text, wenn es keine gibt",',
            ' "achten": ["höchstens drei konkrete Beobachtungspunkte, je mit der Zahl, auf die es ankäme"]}',
            '„widerspruch" ist keine Zusammenfassung: Sage nie die Zahlen aus „text" mit anderen Worten noch einmal.',
            'Trägt der Text die Spannung schon, lass das Feld leer — eine Wiederholung ist schlechter als ein leeres Feld,',
            'sie verdoppelt jede Zahl auf der Seite und in jeder Mail.',
