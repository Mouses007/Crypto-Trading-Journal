...
- **`shared/leverageMap.js`** — The leverage map: **a model, not a measurement**. From the open-interest history it estimates where leveraged positions would be force-liquidated — rising OI in a period means positions were opened there, the candle says at what price, taker volume says in which direction. Moved here from `src/utils/` on 05.09.2026: it was never browser-bound (no DOM, no axios, no Vue), and the KI tile Handelslage needs the zones server-side. Three consumers now share one model — browser (Liquidationskarte), server (`hebelzonen.js`) and the backtest script. `mmr` is a FRACTION here as everywhere (see `liquidation.js`). Its largest fiction is stated in the header: every contract has two sides, and the model assumes only one of them is leveraged. It fetches nothing and draws nothing — drawing is `src/utils/leverageMapRenderer.js`, which stays in `src/` (canvas).
- **`server/hebelzonen.js`** — Turns that map into two or three readable sentences for the Handelslage: the nearest dense zone above and below the price, how the mass is split, and the limits of the estimate. `verdichteHebelkarte` is pure and self-tested, `holeHebelzonen` does the fetching. Uses the **same** `holeLeverageMapPunkte` as the Liquidationskarte tile, so a run normally hits a warm cache and costs no Binance weight. Two traps guard the selftest: the side semantics (below the price sit long liquidations = forced SELLS, above sit shorts = BUYS — swapped, the line reads perfectly plausible and inverts half the assessment) and the unit (`map.long[k][r]` is a COIN amount; without multiplying by the row price BTC shows "84 USD" instead of "84 Mio USD"). Window is the full depth the endpoint gives (~41,6 h): measured 05.09.2026, coverage scales almost linearly with it — 24 h yields 4,5 %, the full depth 11 %. The `duenn` threshold is 5 %, not the renderer's 20 %, because the large markets land at 7–16 % and a warning that always shows is one nobody reads.
- **`server/handelsbild.js`** / **`server/handelslage.js`** — Tile „Handelslage", the sister of the Gesamtlage with a deliberately different horizon: not „where are we in the cycle" but „what does the next few hours hold". `handelsbild.js` is the pure part (candles → day figures, payloads → prompt lines, answer validation) and is covered by `__selftest-handelsbild.mjs`; `handelslage.js` collects ~12 sources in parallel, each behind a 12 s deadline, and calls the model. Its key figure is the **Bewegungsvorrat**: today's range against the MEDIAN of the last ten days — a market that has done 200 % of its usual day is no continuation candidate however clean the trend looks. Median, not mean, because a single crash day lifts the yardstick so far that every normal day afterwards looks quiet. Output is conditions (if/then, bound to a number) and marks at which the reading becomes void — no entries, no price targets.
